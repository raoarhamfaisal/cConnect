import { _ as N, a as A } from "./FeaturesGrid.6b2f0a08.js";
import {
  r as j,
  u as H,
  ac as P,
  c as C,
  _ as T,
  ae as z,
  af as U,
  b as c,
  e as s,
  f as x,
  h as e,
  w as o,
  i,
  t as m,
  x as B,
  s as f,
  J as t,
  F as V,
  U as _,
  a as L,
  g as l,
  H as W,
  L as h,
} from "./app.34e7c902.js";
import { t as g } from "./tContractorWord.ed390c35.js";
import { t as b } from "./tContractorWhite.1a9c0892.js";
import { _ as u } from "./ResponsiveNavLink.98ed2cc3.js";
import { S as M } from "./ScrollToLink.7c70f853.js";
import "./Checkbox.331f80e3.js";
import "./InputError.10c3be20.js";
import "./InputLabel.b83518b5.js";
import "./PrimaryButton.236dcb67.js";
import "./TextInput.14898be7.js";
import "./_plugin-vue_export-helper.cdc0426e.js";
const $ = { class: "relative h-screen bg-white" },
  R = e(
    "img",
    {
      src: "/images/pages/bg_a_xl_white.jpg",
      class: "absolute object-cover w-full h-full",
    },
    null,
    -1
  ),
  Y = { class: "absolute top-0 left-0 right-0 z-20 bg-blue-30-rgba" },
  D = { class: "container max-w-7xl px-6 py-2 mx-auto md:px-12" },
  G = { class: "flex items-center justify-between" },
  I = { class: "flex items-center justify-start" },
  J = { class: "flex items-center justify-end" },
  E = { key: 0 },
  O = { key: 1 },
  Q = { class: "-mr-4 sm:-mr-2 flex items-center" },
  q = {
    class: "h-6 w-6",
    stroke: "currentColor",
    fill: "none",
    viewBox: "0 0 24 24",
  },
  K = { class: "" },
  X = { class: "pt-4 pb-2 pl-3 border-b-2 border-gray-400" },
  Z = { class: "font-bold text-base text-gray-800" },
  ee = { key: 0 },
  te = { key: 1 },
  se = { key: 0, class: "font-medium text-sm text-gray-500" },
  oe = { class: "pt-2 pb-3 space-y-1" },
  re = { class: "pb-1 border-t-2 border-gray-400" },
  le = { class: "mt-3 space-y-1" },
  ie = {
    key: 2,
    class: "pt-2 pb-3 space-y-1 border-b-2 border-t-2 border-gray-400",
  },
  ne = {
    class: "h-6 w-6",
    stroke: "currentColor",
    fill: "none",
    viewBox: "0 0 24 24",
  },
  ae = {
    class:
      "container relative max-w-7xl z-10 flex items-center px-4 xs:px-6 pt-32 pb-5 mx-auto md:px-10 xl:py-30",
  },
  de = {
    class:
      "relative z-10 flex flex-col items-start min-[500px]:w-3/5 md:w-3/5 xl:w-2/5 bg-blue-30-rgba rounded-xl border border-blue-rgba shadow-lg p-2 py-4 px-4",
  },
  ce = { class: "sm:hidden text-4xl font-bold tracking-wide text-center" },
  ue = e(
    "span",
    { class: "font-bold text-slate-300 uppercase" },
    " Better Construction! ",
    -1
  ),
  xe = e(
    "h1",
    {
      class:
        "mt-4 text-4xl sm:text-5xl font-bold text-white tracking-tighter capitalize",
    },
    " Want access to the most powerful tool in construction? ",
    -1
  ),
  he = e(
    "span",
    {
      class:
        "text-lg font-bold text-white uppercase transition-colors group-hover:font-extrabold group-active:text-indigo-500",
    },
    " Get Started ",
    -1
  ),
  fe = e(
    "span",
    {
      class:
        "ml-4 flex-shrink-0 rounded-full border border-current bg-white p-2 text-indigo-600 group-active:text-indigo-500",
    },
    [
      e(
        "svg",
        {
          class: "h-5 w-5",
          xmlns: "http://www.w3.org/2000/svg",
          fill: "none",
          viewBox: "0 0 24 24",
          stroke: "currentColor",
        },
        [
          e("path", {
            "stroke-linecap": "round",
            "stroke-linejoin": "round",
            "stroke-width": "2",
            d: "M17 8l4 4m0 0l-4 4m4-4H3",
          }),
        ]
      ),
    ],
    -1
  ),
  ge = { class: "relative" },
  pe = {
    class:
      "container lg:max-w-4xl xl:max-w-7xl z-20 mx-auto mt-4 md:px-12 py-10 sm:px-20 lg:px-8 bg-slate-30-rgba text-gray-700 rounded-xl",
  },
  me = { key: 0, class: "max-w-lg" },
  ve = { class: "text-3xl font-bold sm:text-4xl" },
  be = e("br", null, null, -1),
  _e = { class: "text-center text-4xl sm:text-6xl" },
  we = e(
    "p",
    {
      class:
        "mt-4 text-center text-3xl sm:text-4xl font-extrabold text-slate-900",
    },
    " To Increase Your Bottom Line & Peace of Mind! ",
    -1
  ),
  ye = e(
    "p",
    { class: "mt-4 text-center text-2xl font-semibold text-gray-800" },
    [
      t(
        " From Searching for Locally Vetted Sub-Contractors, to Finding Jobs, Getting Bids... "
      ),
      e("br"),
      t("to Free Advertising. "),
    ],
    -1
  ),
  ke = {
    key: 1,
    id: "loginHere",
    class: "grid grid-cols-1 md:grid-cols-2 mt-0 md:mt-0 gap-6 md:gap-2",
  },
  je = { class: "flex flex-col flex-shrink-0 justify-center items-center" },
  Ce = { class: "text-3xl font-bold sm:text-4xl md:max-lg:text-3xl" },
  Be = e("br", null, null, -1),
  Le = { class: "text-center text-4xl sm:text-6xl md:max-lg:text-4xl" },
  Me = e(
    "p",
    {
      class:
        "mt-4 text-center text-3xl sm:text-4xl font-extrabold text-slate-900",
    },
    " To Increase Your Bottom Line & Peace of Mind! ",
    -1
  ),
  Fe = e(
    "p",
    { class: "mt-4 text-center text-2xl font-semibold text-gray-800" },
    [
      t(
        " From Searching for Locally Vetted Sub-Contractors, to Finding Jobs, Getting Bids... "
      ),
      e("br"),
      t("to Free Advertising. "),
    ],
    -1
  ),
  Se = {
    class:
      "flex flex-col flex-shrink-0 justify-center items-center p-3 bg-stone-100 border-2 border-blue-rgba rounded-xl shadow-xl",
  },
  Ne = e(
    "div",
    { class: "mx-auto pt-0" },
    [
      e(
        "p",
        { class: "text-green-800 text-center font-bold pb-2 text-xl" },
        " Login in to Join The Private Community For the Construction Trades! "
      ),
    ],
    -1
  ),
  Ae = {
    class: "relative z-20 bg-white md:mt-24 lg:mt-28 xl:mt-32 2xl:mt-40",
    id: "whytContractor",
  },
  He = { id: "cta", class: "relative bg-red-600" },
  Pe = { class: "" },
  Te = {
    class:
      "container flex flex-col-reverse md:flex-row items-center justify-around mx-auto my-0 p-6 space-y-10",
  },
  ze = { class: "flex flex-col space-y-2 pt-6 md:pt-0 px-4 md:px-0" },
  Ue = { class: "" },
  Ve = { class: "capitalize text-xl font-semibold tracking-tighter list-disc" },
  We = e("li", null, "Locate subs to hire", -1),
  $e = e("li", null, "Find local work & job bids", -1),
  Re = e("li", null, "Be aware of whats up in your area!", -1),
  Ye = e("li", null, "Check for slippery customers!", -1),
  De = e("li", null, "Questions & Answers!", -1),
  Ge = e("li", null, "Low cost targeted advertising!", -1),
  Ie = e("li", null, "Mentoring: go the next level with Mentoring!", -1),
  Je = { class: "lg:max-w-md" },
  Ee = e("span", { class: "font-bold italic" }, '"No-Brainer Pricing"', -1),
  Oe = { class: "font-extrabold text-white normal-case" },
  Qe = { class: "flex flex-col justify-center" },
  qe = _(
    '<div class="text-3xl text-black font-bold"><p class="tracking-tight">Your Top Tool For Only</p></div><div class="flex flex-row justify-center"><div class="flex text-5xl justify-start font-extrabold text-green-rgba"><span class="pt-10">$</span></div><div class="price"><span>29</span></div><div class="flex text-xl justify-end items-end italic text-blue-rgba pb-10"><span>/month</span></div></div>',
    2
  ),
  Ke = { class: "flex justify-center items-end" },
  Xe = { class: "bg-blue-rgba" },
  Ze = {
    class:
      "container flex flex-col-reverse md:flex-row justify-between py-12 px-6 mx-auto gap-x-3 md:gap-x-5 space-y-8 md:space-y-0",
  },
  et = {
    class:
      "flex flex-col-reverse items-center justify-between space-y-12 md:flex-col md:space-y-0 md:items-start",
  },
  tt = e(
    "div",
    { class: "mx-auto my-6 text-center first-letter: text-white md:hidden" },
    " Copyright \xA9 2023, All Rights Reserved ",
    -1
  ),
  st = _(
    '<div class="flex justify-center space-x-4"><a href="#"><img src="/images/icons/icon-facebook.svg" alt="" class="h-8"></a><a href="#"><img src="/images/icons/icon-twitter.svg" alt="" class="h-8"></a><a href="#"><img src="/images/icons/icon-instagram.svg" alt="" class="h-8"></a></div>',
    1
  ),
  ot = _(
    '<div class="flex justify-evenly space-x-10"><div class="flex flex-col space-y-3 text-white"><a href="#" class="hover:text-blue-400">Home</a><a href="#" class="hover:text-blue-400">Pricing</a><a href="#" class="hover:text-blue-400">Contact Us</a></div><div class="flex flex-col space-y-3 text-white"><a href="/careers" class="hover:text-blue-400"> Careers</a><a href="/terms-of-service" class="hover:text-blue-400"> Terms of Service</a><a href="/privacy-policy" class="hover:text-blue-400"> Privacy Policy</a></div></div>',
    1
  ),
  rt = { class: "flex flex-col justify-between" },
  lt = { key: 0, class: "flex justify-center space-x-3" },
  it = e(
    "span",
    {
      class:
        "text-lg font-bold text-white uppercase transition-colors group-hover:font-extrabold group-active:text-indigo-500",
    },
    " Get Started ",
    -1
  ),
  nt = e(
    "span",
    {
      class:
        "ml-4 flex-shrink-0 rounded-full border border-current bg-white p-2 text-indigo-600 group-active:text-indigo-500",
    },
    [
      e(
        "svg",
        {
          class: "h-5 w-5",
          xmlns: "http://www.w3.org/2000/svg",
          fill: "none",
          viewBox: "0 0 24 24",
          stroke: "currentColor",
        },
        [
          e("path", {
            "stroke-linecap": "round",
            "stroke-linejoin": "round",
            "stroke-width": "2",
            d: "M17 8l4 4m0 0l-4 4m4-4H3",
          }),
        ]
      ),
    ],
    -1
  ),
  at = { key: 1, class: "flex justify-center space-x-3" },
  dt = e(
    "span",
    {
      class:
        "text-lg font-bold text-white uppercase transition-colors group-hover:font-extrabold group-active:text-indigo-500",
    },
    " News Feed ",
    -1
  ),
  ct = e(
    "span",
    {
      class:
        "ml-4 flex-shrink-0 rounded-full border border-current bg-white p-2 text-indigo-600 group-active:text-indigo-500",
    },
    [
      e(
        "svg",
        {
          class: "h-5 w-5",
          xmlns: "http://www.w3.org/2000/svg",
          fill: "none",
          viewBox: "0 0 24 24",
          stroke: "currentColor",
        },
        [
          e("path", {
            "stroke-linecap": "round",
            "stroke-linejoin": "round",
            "stroke-width": "2",
            d: "M17 8l4 4m0 0l-4 4m4-4H3",
          }),
        ]
      ),
    ],
    -1
  ),
  ut = e(
    "div",
    {
      class:
        "hidden text-white md:block mx-auto my-6 text-center first-letter:",
    },
    " Copyright \xA9 2023, All Rights Reserved ",
    -1
  ),
  jt = {
    __name: "index copy",
    props: { showit: Boolean },
    setup(r) {
      const a = j(!1),
        w = H();
      P({ email: "", password: "", remember: !1 });
      const v = j(null),
        F = C(() =>
          L().props.value.auth.user
            ? L().props.value.auth.user.appeals_privileges === 1
            : !1
        ),
        n = C(() => w.state.profile.profile),
        y = (d) => {
          v.value && !v.value.contains(d.target) && (a.value = !1);
        },
        k = () => {
          a.value = !a.value;
        };
      return (
        T(async () => {
          z() &&
            (console.log("inside token"),
            await w.dispatch("profile/fetchProfile")),
            document.addEventListener("click", y);
        }),
        U(() => {
          document.removeEventListener("click", y);
        }),
        (d, p) => (
          l(),
          c(
            V,
            null,
            [
              s(x(W), { title: "Welcome" }),
              e("div", $, [
                R,
                e("header", Y, [
                  e("nav", D, [
                    e("div", G, [
                      e("div", I, [
                        s(
                          x(h),
                          {
                            href: "/",
                            class:
                              "text-2xl sm:text-5xl md:text-5xl lg:text-6xl xl:text-7xl font-bold tracking-wide text-center",
                          },
                          { default: o(() => [s(g)]), _: 1 }
                        ),
                      ]),
                      e("div", J, [
                        s(
                          M,
                          {
                            href: "#whytContractor",
                            class:
                              "hidden md:block mx-3 text-lg font-bold text-white hover:text-blue-rgba cursor-pointer hover:underline hover:underline-offset-8",
                          },
                          { default: o(() => [t(" Why tContractor ")]), _: 1 }
                        ),
                        s(
                          x(h),
                          {
                            href: "#",
                            class:
                              "hidden lg:block mx-3 text-lg font-bold text-white hover:text-blue-rgba cursor-pointer hover:underline hover:underline-offset-8",
                          },
                          { default: o(() => [t(" Contact Us ")]), _: 1 }
                        ),
                        r.showit
                          ? (l(),
                            c("div", E, [
                              s(
                                x(h),
                                {
                                  href:
                                    n.value && n.value.is_payment_verified
                                      ? d.route("post")
                                      : "/profile-setup",
                                  class:
                                    "block flex justify-center items-center mx-2 p-3 px-6 font-bold rounded-xl text-white bg-green-600 hover:bg-green-800 border-green-600",
                                },
                                { default: o(() => [t(" News Feed ")]), _: 1 },
                                8,
                                ["href"]
                              ),
                            ]))
                          : i("", !0),
                        r.showit
                          ? i("", !0)
                          : (l(),
                            c("div", O, [
                              s(
                                M,
                                {
                                  href: "#loginHere",
                                  class:
                                    "block flex justify-center items-center p-1 px-3 sm:p-2 mx-2 sm:px-6 font-bold rounded-xl text-xs sm:text-base text-white bg-blue-800 hover:bg-green-800 border-2 shadow-lg border-green-600",
                                },
                                { default: o(() => [t(" Login ")]), _: 1 }
                              ),
                            ])),
                        e("div", Q, [
                          e(
                            "button",
                            {
                              ref_key: "dropdownMenu",
                              ref: v,
                              onClick: k,
                              class:
                                "inline-flex items-center justify-center p-2 rounded-md text-black text-gray-500 bg-gray-100 focus:outline-none focus:bg-gray-100 focus:text-gray-500 transition duration-150 ease-in-out",
                            },
                            [
                              (l(),
                              c("svg", q, [
                                e(
                                  "path",
                                  {
                                    class: m({
                                      hidden: a.value,
                                      "inline-flex": !a.value,
                                    }),
                                    "stroke-linecap": "round",
                                    "stroke-linejoin": "round",
                                    "stroke-width": "2",
                                    d: "M4 6h16M4 12h16M4 18h16",
                                  },
                                  null,
                                  2
                                ),
                                e(
                                  "path",
                                  {
                                    class: m({
                                      hidden: !a.value,
                                      "inline-flex": a.value,
                                    }),
                                    "stroke-linecap": "round",
                                    "stroke-linejoin": "round",
                                    "stroke-width": "2",
                                    d: "M6 18L18 6M6 6l12 12",
                                  },
                                  null,
                                  2
                                ),
                              ])),
                            ],
                            512
                          ),
                        ]),
                      ]),
                    ]),
                    e(
                      "div",
                      {
                        class: m([
                          { block: a.value, hidden: !a.value },
                          "absolute top-16 right-4 sm:right-6 lg:right-20 xl:right-32 2xl:right-80 z-30 px-3 text-left border-b border-gray-400 rounded-xl bg-gray-100 flex",
                        ]),
                      },
                      [
                        e("div", K, [
                          e("div", X, [
                            e("div", Z, [
                              r.showit
                                ? (l(),
                                  c(
                                    "div",
                                    ee,
                                    B(d.$page.props.auth.user.name),
                                    1
                                  ))
                                : i("", !0),
                              r.showit
                                ? i("", !0)
                                : (l(), c("div", te, "Not Logged In")),
                            ]),
                            r.showit
                              ? (l(),
                                c(
                                  "div",
                                  se,
                                  B(d.$page.props.auth.user.email),
                                  1
                                ))
                              : i("", !0),
                          ]),
                          e("div", oe, [
                            s(
                              u,
                              { href: "#whytContractor" },
                              { default: o(() => [t(" Why "), s(g)]), _: 1 }
                            ),
                            r.showit
                              ? (l(),
                                f(
                                  u,
                                  {
                                    key: 0,
                                    href:
                                      n.value && n.value.is_payment_verified
                                        ? d.route("post")
                                        : "/profile-setup",
                                    class: "font-bold",
                                  },
                                  {
                                    default: o(() => [t(" News Feed ")]),
                                    _: 1,
                                  },
                                  8,
                                  ["href"]
                                ))
                              : i("", !0),
                            r.showit
                              ? (l(),
                                f(
                                  u,
                                  {
                                    key: 1,
                                    href:
                                      n.value && n.value.is_payment_verified
                                        ? "/ratings/contractor "
                                        : "/profile-setup",
                                  },
                                  {
                                    default: o(() => [t(" My  Ratings ")]),
                                    _: 1,
                                  },
                                  8,
                                  ["href"]
                                ))
                              : i("", !0),
                          ]),
                          e("div", re, [
                            e("div", le, [
                              r.showit
                                ? (l(),
                                  f(
                                    u,
                                    {
                                      key: 0,
                                      href:
                                        n.value && n.value.is_payment_verified
                                          ? "/profile"
                                          : "/profile-setup",
                                    },
                                    {
                                      default: o(() => [t(" Profile ")]),
                                      _: 1,
                                    },
                                    8,
                                    ["href"]
                                  ))
                                : i("", !0),
                              r.showit
                                ? (l(),
                                  f(
                                    u,
                                    {
                                      key: 1,
                                      href:
                                        n.value && n.value.is_payment_verified
                                          ? "/settings"
                                          : "/profile-setup",
                                    },
                                    {
                                      default: o(() => [t(" Settings ")]),
                                      _: 1,
                                    },
                                    8,
                                    ["href"]
                                  ))
                                : i("", !0),
                              s(
                                u,
                                { href: d.route("index") },
                                { default: o(() => [t(" Contact Us ")]), _: 1 },
                                8,
                                ["href"]
                              ),
                              s(
                                u,
                                { href: d.route("index") },
                                { default: o(() => [t(" About Us ")]), _: 1 },
                                8,
                                ["href"]
                              ),
                              F.value && r.showit
                                ? (l(),
                                  c("div", ie, [
                                    s(
                                      u,
                                      { href: "/admin/regions/contractors" },
                                      {
                                        default: o(() => [
                                          t(" All Contractors "),
                                        ]),
                                        _: 1,
                                      }
                                    ),
                                    s(
                                      u,
                                      { href: "/admin/regions/appealed" },
                                      {
                                        default: o(() => [
                                          t(" Appealed Reviews "),
                                        ]),
                                        _: 1,
                                      }
                                    ),
                                  ]))
                                : i("", !0),
                              r.showit
                                ? (l(),
                                  f(
                                    u,
                                    {
                                      key: 3,
                                      href: d.route("logout"),
                                      method: "post",
                                      as: "button",
                                      onClick:
                                        p[0] ||
                                        (p[0] = (S) => (a.value = !a.value)),
                                    },
                                    {
                                      default: o(() => [t(" Log Out ")]),
                                      _: 1,
                                    },
                                    8,
                                    ["href"]
                                  ))
                                : i("", !0),
                              r.showit
                                ? i("", !0)
                                : (l(),
                                  f(
                                    u,
                                    {
                                      key: 4,
                                      href: d.route("signup"),
                                      as: "button",
                                      class: "text-blue-rgba font-bold",
                                    },
                                    {
                                      default: o(() => [t(" Sign Up ")]),
                                      _: 1,
                                    },
                                    8,
                                    ["href"]
                                  )),
                              r.showit
                                ? i("", !0)
                                : (l(),
                                  f(
                                    u,
                                    {
                                      key: 5,
                                      href: "#loginHere",
                                      as: "button",
                                      onClick:
                                        p[1] ||
                                        (p[1] = (S) => (a.value = !a.value)),
                                    },
                                    { default: o(() => [t(" Log In ")]), _: 1 }
                                  )),
                            ]),
                          ]),
                        ]),
                        e(
                          "button",
                          {
                            onClick: k,
                            class:
                              "self-start inline-flex items-start justify-center p-2 rounded-md text-black hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 focus:text-gray-500 transition duration-150 ease-in-out",
                          },
                          [
                            (l(),
                            c("svg", ne, [
                              e(
                                "path",
                                {
                                  class: m({
                                    hidden: !a.value,
                                    "inline-flex": a.value,
                                  }),
                                  "stroke-linecap": "round",
                                  "stroke-linejoin": "round",
                                  "stroke-width": "2",
                                  d: "M6 18L18 6M6 6l12 12",
                                },
                                null,
                                2
                              ),
                            ])),
                          ]
                        ),
                      ],
                      2
                    ),
                  ]),
                ]),
                e("section", null, [
                  e("div", ae, [
                    e("div", de, [
                      e("h2", ce, [s(g)]),
                      ue,
                      xe,
                      s(
                        x(h),
                        {
                          class:
                            "group flex items-center justify-between rounded-xl border border-red-500 bg-red-500 px-5 py-3 mt-8 transition-colors hover:bg-red-800 focus:outline-none focus:ring",
                          href:
                            r.showit &&
                            n.value &&
                            n.value.is_payment_verified === 0
                              ? "/profile-setup"
                              : d.route("signup"),
                        },
                        { default: o(() => [he, fe]), _: 1 },
                        8,
                        ["href"]
                      ),
                    ]),
                  ]),
                ]),
                e("section", ge, [
                  e("div", pe, [
                    r.showit
                      ? (l(),
                        c("div", me, [
                          e("h2", ve, [
                            t(" Why Do You need "),
                            be,
                            e("span", _e, [s(g), t(" ?")]),
                          ]),
                          we,
                          ye,
                        ]))
                      : i("", !0),
                    r.showit
                      ? i("", !0)
                      : (l(),
                        c("div", ke, [
                          e("div", je, [
                            e("h2", Ce, [
                              t(" Why Do You need "),
                              Be,
                              e("span", Le, [s(g), t(" ? ")]),
                            ]),
                            Me,
                            Fe,
                          ]),
                          e("div", Se, [Ne, s(N)]),
                        ])),
                  ]),
                ]),
                e("section", Ae, [s(A)]),
                e("section", He, [
                  e("div", Pe, [
                    e("div", Te, [
                      e("div", ze, [
                        e("div", Ue, [
                          s(
                            x(h),
                            {
                              href: "/",
                              class:
                                "text-5xl md:text-6xl font-bold tracking-wide text-center",
                            },
                            { default: o(() => [s(b)]), _: 1 }
                          ),
                        ]),
                        e("div", null, [
                          e("ul", Ve, [
                            We,
                            $e,
                            Re,
                            Ye,
                            De,
                            Ge,
                            Ie,
                            e("li", Je, [
                              Ee,
                              t(
                                " \xA0- As contractors ourselves, seems like everyone is trying to sell us something at super - exorbitant prices! "
                              ),
                              e("span", Oe, [t("\xA0Not Here at\xA0 "), s(b)]),
                              t("\xA0... useful tools, decently priced! "),
                            ]),
                          ]),
                        ]),
                      ]),
                      e("div", Qe, [
                        qe,
                        e("div", Ke, [
                          s(
                            x(h),
                            {
                              href:
                                r.showit &&
                                n.value &&
                                n.value.is_payment_verified === 0
                                  ? "/profile-setup"
                                  : d.route("signup"),
                              class:
                                "p-3 px-6 pt-2 text-red bg-white rounded-lg border-white border-spacing-3 shadow-2xl shadow-black align-baseline hover:text-white hover:bg-blue-30-rgba",
                            },
                            { default: o(() => [t("Get Started ")]), _: 1 },
                            8,
                            ["href"]
                          ),
                        ]),
                      ]),
                    ]),
                  ]),
                ]),
                e("footer", Xe, [
                  e("div", Ze, [
                    e("div", et, [
                      tt,
                      e("div", null, [
                        s(
                          x(h),
                          {
                            href: "/",
                            class:
                              "text-5xl lg:text-6xl xl:text-7xl font-bold tracking-wide text-center",
                          },
                          { default: o(() => [s(b)]), _: 1 }
                        ),
                      ]),
                      st,
                    ]),
                    ot,
                    e("div", rt, [
                      r.showit
                        ? i("", !0)
                        : (l(),
                          c("div", lt, [
                            s(
                              x(h),
                              {
                                class:
                                  "group flex items-center justify-between rounded-xl border border-red-500 bg-red-500 px-5 py-3 mt-8 transition-colors hover:bg-red-800 focus:outline-none focus:ring",
                                href:
                                  r.showit &&
                                  n.value &&
                                  n.value.is_payment_verified === 0
                                    ? "/profile-setup"
                                    : d.route("signup"),
                              },
                              { default: o(() => [it, nt]), _: 1 },
                              8,
                              ["href"]
                            ),
                          ])),
                      r.showit
                        ? (l(),
                          c("div", at, [
                            s(
                              x(h),
                              {
                                href: d.route("post"),
                                class:
                                  "group flex items-center justify-between rounded-xl border border-green-600 bg-green-600 hover:bg-green-800 px-5 py-3 mt-8 transition-colors focus:outline-none focus:ring",
                                "preserve-scroll": "",
                              },
                              { default: o(() => [dt, ct]), _: 1 },
                              8,
                              ["href"]
                            ),
                          ]))
                        : i("", !0),
                      ut,
                    ]),
                  ]),
                ]),
              ]),
            ],
            64
          )
        )
      );
    },
  };
export { jt as default };
