  ...
  <div class="relative grid items-center">
    ...
    >
      {{
        ...
        Object.keys(selectedOption).length > 0
          ...
          : (selectedOption && Object.keys(selectedOption).length > 0) ||
            (translations && translations.select_any_option)
      }}
      <Icon
      ...
      >
        ...
        >
          {{ optionsPropForText ? option.name : option }}
        </li>
      </ul>
    </div>
  </div>
...
const selectedOptionId = ref(props.modelValue ?? 0);
const optionSelected = props.options.find(
  (option) => option.id == selectedOptionId.value
);
const selectedOption = ref(
  optionSelected && Object.keys(optionSelected).length > 0 ? optionSelected : {}
);
...
watch(
  () => props.modelValue,
  () => {
    selectedOptionId.value = props.modelValue ?? 0;
    const optionSelected = props.options.find(
      (option) => option.id == selectedOptionId.value
    );
...
const toggleDropdown = () => {
  ...
  if (showDropdown.value) {
    nextTick(() => {
      const selectedIndex = props.options.indexOf(props.modelValue.name);
